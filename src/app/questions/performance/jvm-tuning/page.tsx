    ...
    <div className="min-h-screen bg-gradient-to-br from-red-50 via-orange-50 to-amber-50">
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-6 sm:py-8 lg:py-12">
        <nav className="mb-6 sm:mb-8">
          <Link href="/questions/performance" className="text-red-600 hover:text-red-700 transition-all duration-300 flex items-center gap-2 min-h-[44px] px-4 py-2 rounded-xl hover:bg-red-50 hover:shadow-md group">
            <svg className="w-5 h-5 transform group-hover:-translate-x-1 transition-transform" fill="none" stroke="currentColor" viewBox="0 0 24 24">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M10 19l-7-7m0 0l7-7m-7 7h18" />
            </svg>
            <span className="text-base sm:text-lg font-medium">返回性能优化</span>
          </Link>
        </nav>
        <article className="bg-white rounded-2xl shadow-xl hover:shadow-2xl transition-all duration-300 border border-gray-100 overflow-hidden">
          <div className="bg-gradient-to-r from-red-500 to-orange-500 px-6 sm:px-8 lg:px-10 py-8 sm:py-10 lg:py-12">
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold text-white">
              ⚡ JVM 调优面试题
            </h1>
            <p className="text-red-100 mt-3 text-sm sm:text-base lg:text-lg">性能优化 · GC 调优必备</p>
          </div>
            ...
            <div className="space-y-4 sm:space-y-6 lg:space-y-8">
              {questions.map((item, index) => (
                <div key={index} className="border-b border-gray-100 pb-4 sm:pb-6 lg:pb-8 last:border-0 last:pb-0">
                  ...
                  <div className="bg-gradient-to-br from-red-50 to-orange-50 p-4 sm:p-5 lg:p-6 rounded-xl border border-red-100 hover:shadow-md transition-all duration-300">
